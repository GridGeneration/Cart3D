...
#include <Cart3DAlgorithm/Common/shewchuk_predicates.h>
#include <cmath>
#include <cstdio>
#include <cstdlib>
#ifdef CPU86
...
/* #define cfloat cfloat */ /* float or cfloat */
#define cfloatPRINT cfloatprint
#define cfloatRAND cfloatrand
#define NARROWRAND narrowcfloatrand
#define UNIFORMRAND uniformcfloatrand
...
#define Two_Square(a1, a0, x5, x4, x3, x2, x1, x0) \
	...
	static cfloat splitter;
	static cfloat epsilon; /* = 2^(-p).  Used to estimate roundoff errors. */
	/* A set of coefficients used to calculate maximum roundoff errors.          */
	static cfloat resulterrbound;
	static cfloat ccwerrboundA, ccwerrboundB, ccwerrboundC;
	static cfloat o3derrboundA, o3derrboundB, o3derrboundC;
	static cfloat iccerrboundA, iccerrboundB, iccerrboundC;
	static cfloat isperrboundA, isperrboundB, isperrboundC;

	static  cfloat getepsilon()
	{
		return epsilon;
	}
	...
	/*  floating-point arithmetic is too much like cfloat arithmetic.              */
	/*                                                                           */
	/*  Don't change this routine unless you fully understand it.                */
	/*                                                                           */
	/*****************************************************************************/

    static cfloat exactinit()
	{
		cfloat half;
		cfloat check, lastcheck;
		int every_other;
#ifdef LINUX
		int cword;
#endif /* LINUX */
...
#	else												 /* not SINGLE */
		_control87(_PC_53, _MCW_PC); /* Set FPU control word for cfloat precision. */
#	endif											 /* not SINGLE */
#endif												 /* CPU86 */
#ifdef LINUX
#	ifdef SINGLE
...
#	else					/* not SINGLE */
	/*  cword = 4735; */
		cword = 4722; /* set FPU control word for cfloat precision */
#	endif				/* not SINGLE */
		_FPU_SETCW(cword);
#endif /* LINUX */
	...
	static int grow_expansion(int elen, const cfloat* e, cfloat b, cfloat* h)
		/* e and h can be the same. */
	{
		...
		int eindex;
		cfloat enow;
		INEXACT cfloat bvirt;
		cfloat avirt, bround, around;
	...
	static int grow_expansion_zeroelim(int elen, const cfloat* e, cfloat b, cfloat* h)
		/* e and h can be the same. */
	{
		cfloat Q, hh;
		INEXACT cfloat Qnew;
	...
	static int expansion_sum(int elen, const cfloat* e, int flen, const cfloat* f, cfloat* h)
	...
	static int expansion_sum_zeroelim1(int elen, const cfloat* e, int flen, const cfloat* f, cfloat* h)
		/* e and h can be the same, but f and h cannot. */
	{
		cfloat Q;
		INEXACT cfloat Qnew;
	...
	static int expansion_sum_zeroelim2(int elen, const cfloat* e, int flen, const cfloat* f, cfloat* h)
		/* e and h can be the same, but f and h cannot. */
	{
		cfloat Q, hh;
		INEXACT cfloat Qnew;
	...
	static int fast_expansion_sum(int elen, const cfloat* e, int flen, const cfloat* f, cfloat* h)
		/* h cannot be e or f. */
	{
		cfloat Q;
		INEXACT cfloat Qnew;
	...
	static int fast_expansion_sum_zeroelim(int elen, const cfloat* e, int flen, const cfloat* f, cfloat* h)
		/* h cannot be e or f. */
	{
		cfloat Q;
		INEXACT cfloat Qnew;
	...
	static int linear_expansion_sum(int elen, const cfloat* e, int flen, const cfloat* f, cfloat* h)
		/* h cannot be e or f. */
	{
		cfloat Q, q;
		INEXACT cfloat Qnew;
	...
	static int linear_expansion_sum_zeroelim(int elen, const cfloat* e, int flen, const cfloat* f,
		cfloat* h)
		/* h cannot be e or f. */
	{
		cfloat Q, q, hh;
	...
	static int scale_expansion(int elen, const cfloat* e, cfloat b, cfloat* h)
		/* e and h cannot be the same. */
	{
		INEXACT cfloat Q;
		INEXACT cfloat sum;
		INEXACT cfloat product1;
		cfloat product0;
		int eindex, hindex;
		cfloat enow;
		INEXACT cfloat bvirt;
		...
		INEXACT cfloat c;
		INEXACT cfloat abig;
		cfloat ahi, alo, bhi, blo;
		cfloat err1, err2, err3;
	...
	static int scale_expansion_zeroelim(int elen, const cfloat* e, cfloat b, cfloat* h)
		/* e and h cannot be the same. */
	{
		INEXACT cfloat Q, sum;
		cfloat hh;
		INEXACT cfloat product1;
		cfloat product0;
		int eindex, hindex;
	...
	static int compress(int elen, const cfloat* e, cfloat* h)
	...
	static cfloat estimate(int elen, const cfloat* e)
	{
		cfloat Q;
		int eindex;
	...
	cfloat shewchuk::orient2dfast(const cfloat* pa, const cfloat* pb, const cfloat* pc)
	{
		cfloat acx, bcx, acy, bcy;
	...
	cfloat shewchuk::orient2dexact(const cfloat* pa, const cfloat* pb, const cfloat* pc)
	{
		INEXACT cfloat axby1, axcy1, bxcy1, bxay1, cxay1, cxby1;
		cfloat axby0, axcy0, bxcy0, bxay0, cxay0, cxby0;
		cfloat aterms[4], bterms[4], cterms[4];
		INEXACT cfloat aterms3, bterms3, cterms3;
		cfloat v[8], w[12];
		int vlength, wlength;
	...
	cfloat shewchuk::orient2dslow(const cfloat* pa, const cfloat* pb, const cfloat* pc)
	{
		INEXACT cfloat acx, acy, bcx, bcy;
		cfloat acxtail, acytail;
		cfloat bcxtail, bcytail;
		cfloat negate, negatetail;
		cfloat axby[8], bxay[8];
		INEXACT cfloat axby7, bxay7;
		cfloat deter[16];
		int deterlen;
		...
		INEXACT cfloat c;
		INEXACT cfloat abig;
		cfloat a0hi, a0lo, a1hi, a1lo, bhi, blo;
		cfloat err1, err2, err3;
		INEXACT cfloat _i, _j, _k, _l, _m, _n;
		cfloat _0, _1, _2;
	...
	cfloat shewchuk::orient2dadapt(const cfloat* pa, const cfloat* pb, const cfloat* pc, cfloat detsum)
	{
		INEXACT cfloat acx, acy, bcx, bcy;
		cfloat acxtail, acytail, bcxtail, bcytail;
		INEXACT cfloat detleft, detright;
		cfloat detlefttail, detrighttail;
		cfloat det, errbound;
		cfloat B[4], C1[8], C2[12], D[16];
		INEXACT cfloat B3;
		int C1length, C2length, Dlength;
		cfloat u[4];
		INEXACT cfloat u3;
		INEXACT cfloat s1, t1;
		cfloat s0, t0;
		...
		acx = (cfloat)(pa[0] - pc[0]);
		bcx = (cfloat)(pb[0] - pc[0]);
		acy = (cfloat)(pa[1] - pc[1]);
		bcy = (cfloat)(pb[1] - pc[1]);
	...
	cfloat shewchuk::orient2d(const cfloat* pa, const cfloat* pb, const cfloat* pc)
	{
		cfloat detleft, detright, det;
		cfloat detsum, errbound;
	...
	cfloat shewchuk::orient3dfast(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd)
	{
		cfloat adx, bdx, cdx;
		cfloat ady, bdy, cdy;
		cfloat adz, bdz, cdz;
	...
	cfloat shewchuk::orient3dexact(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd)
	{
		INEXACT cfloat axby1, bxcy1, cxdy1, dxay1, axcy1, bxdy1;
		INEXACT cfloat bxay1, cxby1, dxcy1, axdy1, cxay1, dxby1;
		cfloat axby0, bxcy0, cxdy0, dxay0, axcy0, bxdy0;
		cfloat bxay0, cxby0, dxcy0, axdy0, cxay0, dxby0;
		cfloat ab[4], bc[4], cd[4], da[4], ac[4], bd[4];
		cfloat temp8[8];
		int templen;
		cfloat abc[12], bcd[12], cda[12], dab[12];
		int abclen, bcdlen, cdalen, dablen;
		cfloat adet[24], bdet[24], cdet[24], ddet[24];
		int alen, blen, clen, dlen;
		cfloat abdet[48], cddet[48];
		int ablen, cdlen;
		cfloat deter[96];
		int deterlen;
		int i;
	...
	cfloat shewchuk::orient3dslow(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd)
	{
		INEXACT cfloat adx, ady, adz, bdx, bdy, bdz, cdx, cdy, cdz;
		cfloat adxtail, adytail, adztail;
		cfloat bdxtail, bdytail, bdztail;
		cfloat cdxtail, cdytail, cdztail;
		cfloat negate, negatetail;
		INEXACT cfloat axby7, bxcy7, axcy7, bxay7, cxby7, cxay7;
		cfloat axby[8], bxcy[8], axcy[8], bxay[8], cxby[8], cxay[8];
		cfloat temp16[16], temp32[32], temp32t[32];
		int temp16len, temp32len, temp32tlen;
		cfloat adet[64], bdet[64], cdet[64];
		int alen, blen, clen;
		cfloat abdet[128];
		int ablen;
		cfloat deter[192];
	...
	cfloat shewchuk::orient3dadapt(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd, const cfloat permanent)
	{
		INEXACT cfloat adx, bdx, cdx, ady, bdy, cdy, adz, bdz, cdz;
		cfloat det, errbound;

		INEXACT cfloat bdxcdy1, cdxbdy1, cdxady1, adxcdy1, adxbdy1, bdxady1;
		cfloat bdxcdy0, cdxbdy0, cdxady0, adxcdy0, adxbdy0, bdxady0;
		cfloat bc[4], ca[4], ab[4];
		INEXACT cfloat bc3, ca3, ab3;
		cfloat adet[8], bdet[8], cdet[8];
		int alen, blen, clen;
		cfloat abdet[16];
		int ablen;
		cfloat* finnow, * finother, * finswap;
		cfloat fin1[192], fin2[192];
		int finlength;
		...
		cfloat adxtail, bdxtail, cdxtail;
		cfloat adytail, bdytail, cdytail;
		cfloat adztail, bdztail, cdztail;
		INEXACT cfloat at_blarge, at_clarge;
		INEXACT cfloat bt_clarge, bt_alarge;
		INEXACT cfloat ct_alarge, ct_blarge;
		cfloat at_b[4], at_c[4], bt_c[4], bt_a[4], ct_a[4], ct_b[4];
		int at_blen, at_clen, bt_clen, bt_alen, ct_alen, ct_blen;
		INEXACT cfloat bdxt_cdy1, cdxt_bdy1, cdxt_ady1;
		INEXACT cfloat adxt_cdy1, adxt_bdy1, bdxt_ady1;
		cfloat bdxt_cdy0, cdxt_bdy0, cdxt_ady0;
		cfloat adxt_cdy0, adxt_bdy0, bdxt_ady0;
		INEXACT cfloat bdyt_cdx1, cdyt_bdx1, cdyt_adx1;
		INEXACT cfloat adyt_cdx1, adyt_bdx1, bdyt_adx1;
		cfloat bdyt_cdx0, cdyt_bdx0, cdyt_adx0;
		cfloat adyt_cdx0, adyt_bdx0, bdyt_adx0;
		cfloat bct[8], cat[8], abt[8];
		int bctlen, catlen, abtlen;
		INEXACT cfloat bdxt_cdyt1, cdxt_bdyt1, cdxt_adyt1;
		INEXACT cfloat adxt_cdyt1, adxt_bdyt1, bdxt_adyt1;
		cfloat bdxt_cdyt0, cdxt_bdyt0, cdxt_adyt0;
		cfloat adxt_cdyt0, adxt_bdyt0, bdxt_adyt0;
		cfloat u[4], v[12], w[16];
		INEXACT cfloat u3;
		int vlength, wlength;
		cfloat negate;
		...
		cfloat ahi, alo, bhi, blo;
		cfloat err1, err2, err3;
		INEXACT cfloat _i, _j, _k;
		cfloat _0;

		adx = (cfloat)(pa[0] - pd[0]);
		bdx = (cfloat)(pb[0] - pd[0]);
		cdx = (cfloat)(pc[0] - pd[0]);
		ady = (cfloat)(pa[1] - pd[1]);
		bdy = (cfloat)(pb[1] - pd[1]);
		cdy = (cfloat)(pc[1] - pd[1]);
		adz = (cfloat)(pa[2] - pd[2]);
		bdz = (cfloat)(pb[2] - pd[2]);
		cdz = (cfloat)(pc[2] - pd[2]);
	...
	cfloat shewchuk::orient3d(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd)
	{
		cfloat adx, bdx, cdx, ady, bdy, cdy, adz, bdz, cdz;
		cfloat bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
		cfloat det;
		cfloat permanent, errbound;
	...
	cfloat shewchuk::incirclefast(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd)
	{
		cfloat adx, ady, bdx, bdy, cdx, cdy;
		cfloat abdet, bcdet, cadet;
		cfloat alift, blift, clift;
	...
	cfloat shewchuk::incircleexact(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd)
	{
		INEXACT cfloat axby1, bxcy1, cxdy1, dxay1, axcy1, bxdy1;
		INEXACT cfloat bxay1, cxby1, dxcy1, axdy1, cxay1, dxby1;
		cfloat axby0, bxcy0, cxdy0, dxay0, axcy0, bxdy0;
		...
		cfloat abc[12], bcd[12], cda[12], dab[12];
		int abclen, bcdlen, cdalen, dablen;
		cfloat det24x[24], det24y[24], det48x[48], det48y[48];
		int xlen, ylen;
		cfloat adet[96], bdet[96], cdet[96], ddet[96];
		int alen, blen, clen, dlen;
		cfloat abdet[192], cddet[192];
		int ablen, cdlen;
		cfloat deter[384];
		int deterlen;
		int i;
	...
	cfloat shewchuk::incircleslow(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd)
	{
		INEXACT cfloat adx, bdx, cdx, ady, bdy, cdy;
		cfloat adxtail, bdxtail, cdxtail;
		cfloat adytail, bdytail, cdytail;
		cfloat negate, negatetail;
		INEXACT cfloat axby7, bxcy7, axcy7, bxay7, cxby7, cxay7;
		...
		cfloat temp16[16];
		int temp16len;
		cfloat detx[32], detxx[64], detxt[32], detxxt[64], detxtxt[64];
		int xlen, xxlen, xtlen, xxtlen, xtxtlen;
		cfloat x1[128], x2[192];
		int x1len, x2len;
		cfloat dety[32], detyy[64], detyt[32], detyyt[64], detytyt[64];
		int ylen, yylen, ytlen, yytlen, ytytlen;
		cfloat y1[128], y2[192];
		int y1len, y2len;
		cfloat adet[384], bdet[384], cdet[384], abdet[768], deter[1152];
	...
	cfloat shewchuk::incircleadapt(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd, cfloat permanent)
	{
		INEXACT cfloat adx, bdx, cdx, ady, bdy, cdy;
		cfloat det, errbound;
		...
		cfloat bc[4], ca[4], ab[4];
		INEXACT cfloat bc3, ca3, ab3;
		cfloat axbc[8], axxbc[16], aybc[8], ayybc[16], adet[32];
		int axbclen, axxbclen, aybclen, ayybclen, alen;
		cfloat bxca[8], bxxca[16], byca[8], byyca[16], bdet[32];
		int bxcalen, bxxcalen, bycalen, byycalen, blen;
		cfloat cxab[8], cxxab[16], cyab[8], cyyab[16], cdet[32];
		int cxablen, cxxablen, cyablen, cyyablen, clen;
		cfloat abdet[64];
		int ablen;
		cfloat fin1[1152], fin2[1152];
		cfloat* finnow, * finother, * finswap;
		int finlength;

		cfloat adxtail, bdxtail, cdxtail, adytail, bdytail, cdytail;
		INEXACT cfloat adxadx1, adyady1, bdxbdx1, bdybdy1, cdxcdx1, cdycdy1;
		cfloat adxadx0, adyady0, bdxbdx0, bdybdy0, cdxcdx0, cdycdy0;
		cfloat aa[4], bb[4], cc[4];
		INEXACT cfloat aa3, bb3, cc3;
		INEXACT cfloat ti1, tj1;
		cfloat ti0, tj0;
		cfloat u[4], v[4];
		INEXACT cfloat u3, v3;
		cfloat temp8[8], temp16a[16], temp16b[16], temp16c[16];
		cfloat temp32a[32], temp32b[32], temp48[48], temp64[64];
		int temp8len, temp16alen, temp16blen, temp16clen;
		int temp32alen, temp32blen, temp48len, temp64len;
		cfloat axtbb[8], axtcc[8], aytbb[8], aytcc[8];
		int axtbblen, axtcclen, aytbblen, aytcclen;
		cfloat bxtaa[8], bxtcc[8], bytaa[8], bytcc[8];
		int bxtaalen, bxtcclen, bytaalen, bytcclen;
		cfloat cxtaa[8], cxtbb[8], cytaa[8], cytbb[8];
		int cxtaalen, cxtbblen, cytaalen, cytbblen;
		cfloat axtbc[8], aytbc[8], bxtca[8], bytca[8], cxtab[8], cytab[8];
		int axtbclen, aytbclen, bxtcalen, bytcalen, cxtablen, cytablen;
		cfloat axtbct[16], aytbct[16], bxtcat[16], bytcat[16], cxtabt[16], cytabt[16];
		int axtbctlen, aytbctlen, bxtcatlen, bytcatlen, cxtabtlen, cytabtlen;
		cfloat axtbctt[8], aytbctt[8], bxtcatt[8];
		cfloat bytcatt[8], cxtabtt[8], cytabtt[8];
		int axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen, cytabttlen;
		cfloat abt[8], bct[8], cat[8];
		int abtlen, bctlen, catlen;
		cfloat abtt[4], bctt[4], catt[4];
		int abttlen, bcttlen, cattlen;
		INEXACT cfloat abtt3, bctt3, catt3;
		cfloat negate;
	...
	cfloat shewchuk::incircle(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd)
	{
		cfloat adx, bdx, cdx, ady, bdy, cdy;
		cfloat bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
		cfloat alift, blift, clift;
		cfloat det;
		cfloat permanent, errbound;
	...
	cfloat shewchuk::inspherefast(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd, const cfloat* pe)
	{
		cfloat aex, bex, cex, dex;
		cfloat aey, bey, cey, dey;
		cfloat aez, bez, cez, dez;
		cfloat alift, blift, clift, dlift;
		cfloat ab, bc, cd, da, ac, bd;
		cfloat abc, bcd, cda, dab;
	...
	cfloat shewchuk::insphereexact(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd, const cfloat* pe)
	{
		INEXACT cfloat axby1, bxcy1, cxdy1, dxey1, exay1;
		INEXACT cfloat bxay1, cxby1, dxcy1, exdy1, axey1;
		INEXACT cfloat axcy1, bxdy1, cxey1, dxay1, exby1;
		INEXACT cfloat cxay1, dxby1, excy1, axdy1, bxey1;
		cfloat axby0, bxcy0, cxdy0, dxey0, exay0;
		cfloat bxay0, cxby0, dxcy0, exdy0, axey0;
		cfloat axcy0, bxdy0, cxey0, dxay0, exby0;
		cfloat cxay0, dxby0, excy0, axdy0, bxey0;
		cfloat ab[4], bc[4], cd[4], de[4], ea[4];
		cfloat ac[4], bd[4], ce[4], da[4], eb[4];
		cfloat temp8a[8], temp8b[8], temp16[16];
		int temp8alen, temp8blen, temp16len;
		cfloat abc[24], bcd[24], cde[24], dea[24], eab[24];
		cfloat abd[24], bce[24], cda[24], deb[24], eac[24];
		int abclen, bcdlen, cdelen, dealen, eablen;
		int abdlen, bcelen, cdalen, deblen, eaclen;
		cfloat temp48a[48], temp48b[48];
		int temp48alen, temp48blen;
		cfloat abcd[96], bcde[96], cdea[96], deab[96], eabc[96];
		int abcdlen, bcdelen, cdealen, deablen, eabclen;
		cfloat temp192[192];
		cfloat det384x[384], det384y[384], det384z[384];
		int xlen, ylen, zlen;
		cfloat detxy[768];
		int xylen;
		cfloat adet[1152], bdet[1152], cdet[1152], ddet[1152], edet[1152];
		int alen, blen, clen, dlen, elen;
		cfloat abdet[2304], cddet[2304], cdedet[3456];
		int ablen, cdlen;
		cfloat deter[5760];
		int deterlen;
		int i;
	...
	cfloat shewchuk::insphereslow(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd, const cfloat* pe)
	{
		INEXACT cfloat aex, bex, cex, dex, aey, bey, cey, dey, aez, bez, cez, dez;
		cfloat aextail, bextail, cextail, dextail;
		cfloat aeytail, beytail, ceytail, deytail;
		cfloat aeztail, beztail, ceztail, deztail;
		cfloat negate, negatetail;
		INEXACT cfloat axby7, bxcy7, cxdy7, dxay7, axcy7, bxdy7;
		INEXACT cfloat bxay7, cxby7, dxcy7, axdy7, cxay7, dxby7;
		cfloat axby[8], bxcy[8], cxdy[8], dxay[8], axcy[8], bxdy[8];
		cfloat bxay[8], cxby[8], dxcy[8], axdy[8], cxay[8], dxby[8];
		cfloat ab[16], bc[16], cd[16], da[16], ac[16], bd[16];
		int ablen, bclen, cdlen, dalen, aclen, bdlen;
		cfloat temp32a[32], temp32b[32], temp64a[64], temp64b[64], temp64c[64];
		int temp32alen, temp32blen, temp64alen, temp64blen, temp64clen;
		cfloat temp128[128], temp192[192];
		int temp128len, temp192len;
		cfloat detx[384], detxx[768], detxt[384], detxxt[768], detxtxt[768];
		int xlen, xxlen, xtlen, xxtlen, xtxtlen;
		cfloat x1[1536], x2[2304];
		int x1len, x2len;
		cfloat dety[384], detyy[768], detyt[384], detyyt[768], detytyt[768];
		int ylen, yylen, ytlen, yytlen, ytytlen;
		cfloat y1[1536], y2[2304];
		int y1len, y2len;
		cfloat detz[384], detzz[768], detzt[384], detzzt[768], detztzt[768];
		int zlen, zzlen, ztlen, zztlen, ztztlen;
		cfloat z1[1536], z2[2304];
		int z1len, z2len;
		cfloat detxy[4608];
		int xylen;
		cfloat adet[6912], bdet[6912], cdet[6912], ddet[6912];
		int alen, blen, clen, dlen;
		cfloat abdet[13824], cddet[13824], deter[27648];
		int deterlen;
		int i;
	...
	cfloat shewchuk::insphereadapt(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd, const cfloat* pe,
		cfloat permanent)
	{
		INEXACT cfloat aex, bex, cex, dex, aey, bey, cey, dey, aez, bez, cez, dez;
		cfloat det, errbound;

		INEXACT cfloat aexbey1, bexaey1, bexcey1, cexbey1;
		INEXACT cfloat cexdey1, dexcey1, dexaey1, aexdey1;
		INEXACT cfloat aexcey1, cexaey1, bexdey1, dexbey1;
		cfloat aexbey0, bexaey0, bexcey0, cexbey0;
		cfloat cexdey0, dexcey0, dexaey0, aexdey0;
		cfloat aexcey0, cexaey0, bexdey0, dexbey0;
		cfloat ab[4], bc[4], cd[4], da[4], ac[4], bd[4];
		INEXACT cfloat ab3, bc3, cd3, da3, ac3, bd3;
		cfloat abeps, bceps, cdeps, daeps, aceps, bdeps;
		cfloat temp8a[8], temp8b[8], temp8c[8], temp16[16], temp24[24], temp48[48];
		int temp8alen, temp8blen, temp8clen, temp16len, temp24len, temp48len;
		cfloat xdet[96], ydet[96], zdet[96], xydet[192];
		int xlen, ylen, zlen, xylen;
		cfloat adet[288], bdet[288], cdet[288], ddet[288];
		int alen, blen, clen, dlen;
		cfloat abdet[576], cddet[576];
		int ablen, cdlen;
		cfloat fin1[1152];
		int finlength;
		...
		aex = (cfloat)(pa[0] - pe[0]);
		bex = (cfloat)(pb[0] - pe[0]);
		cex = (cfloat)(pc[0] - pe[0]);
		dex = (cfloat)(pd[0] - pe[0]);
		aey = (cfloat)(pa[1] - pe[1]);
		bey = (cfloat)(pb[1] - pe[1]);
		cey = (cfloat)(pc[1] - pe[1]);
		dey = (cfloat)(pd[1] - pe[1]);
		aez = (cfloat)(pa[2] - pe[2]);
		bez = (cfloat)(pb[2] - pe[2]);
		cez = (cfloat)(pc[2] - pe[2]);
		dez = (cfloat)(pd[2] - pe[2]);
	...
	cfloat shewchuk::insphere(const cfloat* pa, const cfloat* pb, const cfloat* pc, const cfloat* pd, const cfloat* pe)
	{
		cfloat aex, bex, cex, dex;
		cfloat aey, bey, cey, dey;
		cfloat aez, bez, cez, dez;
		cfloat aexbey, bexaey, bexcey, cexbey, cexdey, dexcey, dexaey, aexdey;
		cfloat aexcey, cexaey, bexdey, dexbey;
		cfloat alift, blift, clift, dlift;
		cfloat ab, bc, cd, da, ac, bd;
		cfloat abc, bcd, cda, dab;
		cfloat aezplus, bezplus, cezplus, dezplus;
		cfloat aexbeyplus, bexaeyplus, bexceyplus, cexbeyplus;
		cfloat cexdeyplus, dexceyplus, dexaeyplus, aexdeyplus;
		cfloat aexceyplus, cexaeyplus, bexdeyplus, dexbeyplus;
		cfloat det;
		cfloat permanent, errbound;
